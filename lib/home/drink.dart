import 'package:flutter/material.dart'; // Import do serviço que faz as requisições HTTP
import '../service/drink_service.dart';  
import '../models/bebidas.dart';     // Import do modelo de dados (classe Product)
...
class _DrinkScreenState extends State<DrinksScreen> {
  // VARIÁVEIS DE ESTADO
  // Estas variáveis armazenam o estado atual do componente
  List<Drink> _drinks = [];  // Lista que armazenará os produtos da API
  ...
  Future<void> _loadProducts() async {
    ...
    try {
      ...
      final drinks = await DrinkService.getProducts();
     
      // Atualiza o estado com os dados recebidos da API
      setState(() {
        _drinks = drinks;  // Armazena os produtos na variável de estado
      });
    } catch (e) {
      //  TRATAMENTO DE ERROS
  ...
  Widget _buildBody() {
    ...
    return ListView.builder(
      itemCount: _drinks.length,  // Quantidade de itens na lista
      itemBuilder: (context, index) {
        final drink = _drinks[index];  // Pega o produto atual
        ...
        return Card(
          ...
          child: ListTile(
            leading: Container(
              ...
              child: drink.imagemUrl != null
                  ? Image.network(drink.imagemUrl!)  // Imagem do produto
                  : Icon(Icons.shopping_bag, color: Colors.blue),  // Ícone padrão
            ),
            title: Text(

               drink.descricao,  // Nome do produto vindo da API
              style: TextStyle(
              ...
              children: [
               // Tamanho do produto
                ...
                Row(
                  children: [
                    Text(
                      'R\$${drink.valor.toStringAsFixed(2)}',  // Preço formatado
                      style: TextStyle(
                        fontWeight: FontWeight.bold,
                    ...
                    SizedBox(width: 16),
                    Text(
                      'Estoque: ${drink.quantidade}',  // Quantidade em estoque
            ...
            trailing: IconButton(
              ...
              onPressed: () {
                _showProductDetails(context, drink);  // Mostra detalhes
              },
            ),
          ),
        );
  ...
  void _showProductDetails(BuildContext context, Drink drink) {
    showDialog(
      context: context,
      builder: (context) => AlertDialog(
        title: Text(drink.descricao),
        content: Column(
          ...
          children: [
            SizedBox(height: 16),
            Text(
              'Preço: R\$${drink.valor.toStringAsFixed(2)}',
              style: TextStyle(
                fontWeight: FontWeight.bold,
                fontSize: 18,
                color: Colors.green[700],
            ...
            SizedBox(height: 8),
            Text('Estoque disponível: ${drink.quantidade}'),
          ],
        ),
        actions: [
          TextButton(
